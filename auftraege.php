...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postAction === 'save') {
        ...
        if (empty($positionen)) {
            ...
            header('Location: auftraege.php?action=' . ($data['id'] ? 'edit&id=' . $data['id'] : 'new'));
            exit;
        }

        $result = saveVerkaufsdokument($data, $positionen);
        setFlashMessage($result['success'] ? 'success' : 'danger', $result['success'] ? 'Auftrag gespeichert.' : $result['message']);
    ...
    if ($postAction === 'delete') {
        ...
        setFlashMessage($result['success'] ? 'success' : 'danger', $result['success'] ? 'Auftrag gelöscht.' : $result['message']);
        header('Location: auftraege.php');
        exit;
    }

    if ($postAction === 'umwandeln') {
        $result = umwandelnVerkaufsdokument((int)$_POST['id'], 'rechnung');
        if ($result['success']) {
            setFlashMessage('success', 'Auftrag wurde in eine Rechnung umgewandelt.');
            header('Location: verkaufsrechnungen.php?action=edit&id=' . $result['id']);
        } else {
    ...
    if ($postAction === 'finalisieren') {
        ...
        if ($result['success']) {
            $meldung = 'Auftrag ' . $result['nummer'] . ' finalisiert.';
    ...
    if ($postAction === 'versenden') {
        ...
        setFlashMessage($result['success'] ? 'success' : 'danger', $result['success'] ? 'Auftrag per E-Mail an ' . $result['empfaenger'] . ' versendet.' : $result['error']);
        header('Location: auftraege.php');
        exit;
    }
}
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2"><i class="bi bi-clipboard-check me-2"></i><?= $dokument ? 'Auftrag bearbeiten' : 'Neuer Auftrag' ?></h1>
                    <a href="auftraege.php" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Zurück</a>
                </div>
                ...
                <form method="POST">
                        ...
                        <div class="card-body">
                            <div class="row mb-3">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="date" class="form-control" name="leistungsdatum" value="<?= $dokument['leistungsdatum'] ?? '' ?>">
                                </div>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" name="einleitungstext" rows="2"><?= htmlspecialchars($dokument['einleitungstext'] ?? 'Vielen Dank für Ihren Auftrag. Wir bestätigen wie folgt:') ?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            <label class="form-label">Fällig am (für spätere Rechnung)</label>
                            <input type="date" class="form-control" name="faellig_am" value="<?= $dokument['faellig_am'] ?? '' ?>">
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Schlusstext</label>
                        <textarea class="form-control" name="schlusstext" rows="2"><?= htmlspecialchars($dokument['schlusstext'] ?? '') ?></textarea>
                    </div>
                    <div class="mb-3">
                    ...
                    <a href="auftraege.php" class="btn btn-outline-secondary">Abbrechen</a>
                </form>

                <?php elseif ($action === 'view' && $dokument): ?>
                <!-- Ansicht -->
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2"><i class="bi bi-clipboard-check me-2"></i>Auftrag <?= htmlspecialchars($dokument['nummer'] ?: '(Entwurf)') ?></h1>
                    <div>
                ...
                <!-- Liste -->
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2"><i class="bi bi-clipboard-check me-2"></i>Aufträge</h1>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            ...
                                            <form method="POST" class="d-inline">
                                                ...
                                                <button type="submit" class="btn btn-sm btn-success" onclick="return confirm('Auftrag finalisieren? Danach ist keine Bearbeitung mehr möglich.')">
                                                    <i class="bi bi-check-circle"></i> Finalisieren
                                                </button>
                                            </form>
                                            <form method="POST" class="d-inline" onsubmit="return confirm('Auftrag wirklich löschen?');">
                                                <input type="hidden" name="action" value="delete">
                                                <input type="hidden" name="id" value="<?= $d['id'] ?>">
                                                <button type="submit" class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button>
                                            </form>
                                            <?php else: ?>
                                            <a href="auftraege.php?action=view&id=<?= $d['id'] ?>" class="btn btn-sm btn-outline-secondary"><i class="bi bi-eye"></i></a>
                                            ...
                                            <form method="POST" class="d-inline" onsubmit="return confirm('Auftrag in Rechnung umwandeln?');">
                                                ...
                                                <input type="hidden" name="id" value="<?= $d['id'] ?>">
                                                <button type="submit" class="btn btn-sm btn-outline-success" title="In Rechnung umwandeln">
                                                    <i class="bi bi-arrow-right-circle"></i>
                                                </button>
                                            </form>
                                            <?php endif; ?>
                                            <?php if ($d['status'] !== 'entwurf'): ?>
                                            <a href="wiederkehrende_rechnungen.php?action=new&vorlage_id=<?= $d['id'] ?>" class="btn btn-sm btn-outline-info" title="In wiederkehrende Rechnung umwandeln">
